...
{	
	...
	{
		String txt = (new TranslationTextComponent("tooltype." + stack.getCapability(ToolDefinitionCapability.TOOL_DEF_CAP).orElseGet(null).getToolType().getName())).getFormattedText();
		//String txt = new TranslationTextComponent("item.deepnether.dylithite_ingot").getFormattedText();
		return new StringTextComponent(ForgeRegistries.ITEMS.getValue(new ResourceLocation(stack.getCapability(ToolDefinitionCapability.TOOL_DEF_CAP).orElseGet(null).getHead().getBoundItem())).getName().getFormattedText() + " " + txt);
	}
	...
	public Multimap<String, AttributeModifier> getAttributeModifiers(EquipmentSlotType slot, ItemStack stack)
	{
		...
		map.put(SharedMonsterAttributes.ATTACK_DAMAGE.getName(), new AttributeModifier("attackDamage", stats.getAttackDamage(), Operation.ADDITION));
		return map;
	}
}
