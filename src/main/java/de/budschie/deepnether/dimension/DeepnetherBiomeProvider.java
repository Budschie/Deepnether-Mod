...
public class DeepnetherBiomeProvider extends BiomeProvider
{
	...
	public DeepnetherBiomeProvider(long seed, float biomeSize, DeepnetherBiomeBase... biomes) 
	{
		super(null);
	...
	public Biome[] getBiomes(int x, int z, int width, int depth) 
	{
		...
		Biome[] biomesToOutput = new Biome[width*depth];
		...
		for(int xLoop = 0; xLoop < width; xLoop++)
		{
			...
			//	testNoiseArr[i][j] = processedNoiseOut;
				...
				biomesToOutput[pos] = this.biomes.get(processedNoiseOut);
	...
	public Vec3d getFogColor(int x, int z, int radius)
	{
		...
		for(int xLoop = 0; xLoop < radius*2; xLoop++)
		{
			...
			{
				Vec3d temp = biomes[pos].getFogColor();
				rMixed += temp.x;
				gMixed += temp.y;
				bMixed += temp.z;
				pos++;
