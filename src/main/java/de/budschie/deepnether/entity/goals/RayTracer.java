...
public class RayTracer 
{
	...
	public RayTracer(Vector3d start, Vector3d end, boolean shouldGrow)
	{
		this.start = start;
		this.end = end;
		float amount = 1.0f;
	...
	public RayTracerResult run(World world, float quality)
	{
		...
		{
			Vector3d positionNew = new Vector3d(interpolate(start.x, end.x, i), interpolate(start.y, end.y, i), interpolate(start.z, end.z, i));
